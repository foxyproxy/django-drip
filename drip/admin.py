...
def create_drip_admin(drip_model_class, query_set_rule_inline):
    ...
    class DripAdmin(admin.ModelAdmin):
        ...
        def timeline(self, request, drip_id, into_past, into_future):
            ...
            drip = get_object_or_404(drip_model_class, id=drip_id)
...
def create_sent_drip_admin(sent_drip_model):
    class SentDripAdmin(admin.ModelAdmin):
        list_display = [f.name for f in sent_drip_model._meta.fields]
        ordering = ['-id']
    return SentDripAdmin


admin.site.register(Drip, create_drip_admin(Drip, QuerySetRuleInline))
admin.site.register(SentDrip, create_sent_drip_admin(SentDrip))
